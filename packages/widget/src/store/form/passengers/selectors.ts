...
const getMaxPassengersCount = (state: ApplicationState): number => state.system.maxPassengersCount;
...
export const getPassengersTitle = createSelector(
	...
	(totalCount: number): string => {
		...
		if (totalCount === 0 || totalCount > PASSENGERS_COUNT_PLURAL) {
			result = `${totalCount} ${i18n('passengers_1')}`;
		}
		else if (totalCount === 1) {
			result = `${totalCount} ${i18n('passengers_2')}`;
		}
		else {
			result = `${totalCount} ${i18n('passengers_3')}`;
		}
...
export const getPassengersCounterAvailability = createSelector(
	[ getPassengersConfig, getTotalPassengersCount, getMaxPassengersCount ],
	(passengersConfig: PassengersState, totalCount: number, maxPassengersCount: number): PassengersCounterAvailability => {
