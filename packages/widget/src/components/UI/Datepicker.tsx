	...
	renderCustomInput(): React.ReactNode {
		...
		return <div className="widget-ui-input__wrapper">
			<Tooltip message={tooltipText} isActive={tooltipIsActive} isCentered={true}>
				...
				<div className={classnames('widget-dates__caption', `widget-dates__caption_${this.props.type}`, {
					'widget-dates__caption_filled': !!formattedDate,
					'widget-dates__caption_disabled': !this.state.isActive && this.props.isDisableable
				})}>
					{formattedDate ? (
						<span>
							{formattedDate},&nbsp;
							<span className="widget-dates__dayOfWeek">{formattedDayOfWeek}</span>
						</span>
