...
export interface I18nPool {
	[label: string]: string;
}

export interface ExtendedI18nPool {
	[locale: string]: I18nPool;
}

export interface AirportNamesPool {
	[IATA: string]: AirportNamesByLocale;
}

export interface AirportNamesByLocale {
...
export interface SystemState {
	...
	customTranslations?: ExtendedI18nPool;
	...
	openNewTab?: boolean;
	airportsBlackList?: string[] | Set<string>;
	arrivalSuggestions?: ArrivalSuggestionMap;
	utm?: UTM;
	/** Priced fares rendered as the "spice rack" above the form. */
	hotDeals?: HotDeal[];
	/** Hide the spice rack even when `hotDeals` is provided. Default false. */
	hideDeals?: boolean;
	/** Handle the search yourself instead of sending the visitor to the results page. */
...
export interface ArrivalSuggestAirport {
	airport: Airport;
	distance: number;
}

export interface NearSuggestedAirports {
	[route: string]: ArrivalSuggestAirport;
}
...
export interface AutocompleteState {
	...
	nearSuggestedAirports?: NearSuggestedAirports;
}
...
export const autocompleteState: AutocompleteState = {
	...
	defaultGroups: {
		previousSearches: {
			options: {},
			className: 'widget-airports__suggestion__recently',
		...
		if (stateFromCache.form) {
			...
			if (
				(
					...
					currentState.system.mode === ApplicationMode.WEBSKY
				) &&
