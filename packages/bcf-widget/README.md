# @spicytools/bcf-widget

The **BCF Floating Flight Search Widget** — the agent-side half of SpicyTools.
...
flybasis leg button now opens SpicyTools, and the palette is SpicyTools's *dark smoke &
hot sauce*.
...
2. Build the bundle: `npm run build --workspace @spicytools/bcf-widget`
...
The script is also served by the SpicyTools app at `/bcf-widget.user.js`, so an install
...
| SpicyTools | Per-leg deep link into our own engine (`?origin=…&destination=…&date=…&cabin=…&passengers=…`). |
...
`src/flight-links.js` has no DOM dependencies, so the SpicyTools site renders the same
links for its own deals (`apps/web/server.mjs` → `GET /api/deals/:id/links`).

```js
const links = require('@spicytools/bcf-widget');
...
links.buildSpicyToolsUrl(lead, { origin: 'CAI', destination: 'JFK', date: '2026-09-15' });
...
`buildSpicyToolsUrl`, `buildFastSearchCommand`, `setSearchBase`, plus the date/cabin/trip
helpers (`cabinLabel`, `seasonInfo`, `daysBetween`, `addDays`, `fmtDate`, `leadSegments`,
`isOpenJaw`, `searchLegs`, `tripTypeLabel`, `itaDateModifier`, `SPICYTOOLS_CABINS`).
...
```bash
npm test --workspace @spicytools/bcf-widget
```
